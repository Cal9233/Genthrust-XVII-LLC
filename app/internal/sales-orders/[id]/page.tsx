...
export default function InternalSalesOrderDetailPage() {
  ...
  async function loadData() {
    ...
    setError(null)
    try {
      const res = await fetch(`/api/internal/sales-orders/${id}`)
      if (!res.ok) {
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-6">
        <div className="flex items-start justify-between mb-4">
          <div>
            <h1 className="text-2xl font-extrabold text-navy-900">{order.so_number}</h1>
            <p className="text-slate-500 text-sm mt-1">Sales Order</p>
          </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
          <div>
            ...
            <p className="text-navy-900 font-semibold">{order.customer_name || '—'}</p>
          </div>
          <div>
            <p className="text-slate-500 font-medium">Customer PO</p>
            <p className="text-navy-900 font-semibold">{order.customer_po || '—'}</p>
          </div>
          <div>
          ...
          <div>
            ...
            <p className="text-navy-900 font-semibold">{order.track_no || '—'}</p>
          </div>
          <div>
          ...
          <div>
            ...
            <p className="text-navy-900 font-semibold">{formatCurrency(order.subtotal)}</p>
          </div>
          <div>
            <p className="text-slate-500 font-medium">Discount</p>
            <p className="text-navy-900 font-semibold">{formatCurrency(order.total_discount)}</p>
          </div>
          <div>
            <p className="text-slate-500 font-medium">VAT</p>
            <p className="text-navy-900 font-semibold">{formatCurrency(order.total_vat)}</p>
          </div>
          <div>
